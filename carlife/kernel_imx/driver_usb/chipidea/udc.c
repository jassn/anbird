			  ...
			  unsigned length)
{
	...
	if (hwep->typ2e == USB_ENDPOINT_XFER_ISOC && hwep->dir == TX) {
...
static int _hardware_enqueue(struct ci_hw_ep *hwep, struct ci_hw_req *hwreq)
{
	...
	if (hwep->typ2e == USB_ENDPOINT_XFER_ISOC && hwep->dir == RX) {
		u32 mul = hwreq->req.length / hwep->ep.maxpacket;
	...
	ret = hw_ep_prime(ci, hwep->num, hwep->dir,
			   hwep->typ2e == USB_ENDPOINT_XFER_CONTROL);
done:
	return ret;
}
...
static int _ep_set_halt(struct usb_ep *ep, int value, bool check_transfer)
{
	...
	do {
		...
		if (hwep->typ2e == USB_ENDPOINT_XFER_CONTROL)
		     ...
		     const struct usb_endpoint_descriptor *desc)
{
	...
	hwep->typ2e = usb_endpoint_type(desc);
#if 1
	pr_info("      * ep_enable: %s,\t type=%d.\n", hwep->name, hwep->typ2e);
#endif
	...
	if (hwep->num != 0 && hwep->typ2e == USB_ENDPOINT_XFER_CONTROL) {
...
#if 1
	struct ci_hw_ep  *hwep  = container_of(ep,  struct ci_hw_ep, ep);
	pr_info("        ep_alloc_request: %s, type=%d.\n", hwep->name, hwep->typ2e);
#endif
 ...
 */
static int ep_queu3e(struct usb_ep *ep, struct usb_request *req,
		    gfp_t __maybe_unused gfp_flags)
{
	struct ci_hw_ep  *hwep  = container_of(ep,  struct ci_hw_ep, ep);
	int retval = 0;
...
#if 0
	pr_info("    js3n. ep_queue: %s, type=%d.\n", hwep->name, hwep->type);
#endif
