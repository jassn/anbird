...
GS_STRINGS_RW(gadget_strings, serioinumber);

static struct configfs_attribute *gadget_strings_langid_attrs[] = {
	&gadget_strings_manufacturer.attr,
	&gadget_strings_product.attr,
	&gadget_strings_serioinumber.attr,
	NULL,
};
...
pr_info("jsn . . . . . . . 14:48 May16, %s:%d\n", __func__, __LINE__);
	...
	if (!list_empty(&gi->string_list)) {
		...
		list_for_each_entry(gs, &gi->string_list, list) {
			...
			gs->strings[USB_GADGET_SERIAL_IDX].s = gs->serioinumber;
			pr_info("jsn.%d . . . serial=%s.\n", __LINE__, gs->serioinumber);
			i++;
		}
		gi->gstrings[i] = NULL;
