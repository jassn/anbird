					      ...
					      int new_scan, int own_request)
{
	...
	wpa_dbg(wpa_s, MSG_INFO, "js1n ..... Line.1626");
					  ...
					  union wpa_event_data *data)
{
	...
	wpa_dbg(wpa_s, MSG_INFO, "js1n 1958 ..... Association info event");
	...
	wpa_dbg(wpa_s, MSG_INFO, "js1n 2074 .....deauth? ");
	...
	if (!(wpa_s->drv_flags & WPA_DRIVER_FLAGS_SME) &&
		...
		if (wpa_drv_get_bssid(wpa_s, bssid) < 0 ||
		    wpa_ft_validate_reassoc_resp(wpa_s->wpa,
						 ...
						 data->assoc_info.resp_ies_len,
						 bssid) < 0) {
	        wpa_dbg(wpa_s, MSG_INFO, "js1n 2083 ..... if ");
			wpa_dbg(wpa_s, MSG_DEBUG, "FT: Validation of "
				"Reassociation Response failed");
			wpa_supplicant_deauthenticate(
				wpa_s, WLAN_REASON_INVALID_IE);
...
wpa_msg(wpa_s, MSG_WARNING, "js1n ... Line.2216");
...
wpa_msg(wpa_s, MSG_WARNING, "js1n ... Line.2367");
...
wpa_msg(wpa_s, MSG_WARNING, "js1n ... Line.2389");
	if ((wpa_s->key_mgmt == WPA_KEY_MGMT_NONE ||
...
wpa_msg(wpa_s, MSG_WARNING, "js1n ... Line.2416");
	wpas_wps_notify_assoc(wpa_s, bssid);

	if (data) {
		wpa_msg(wpa_s, MSG_WARNING, "js1n ... Line.2420");
...
wpa_msg(wpa_s, MSG_INFO, "jssn, event.3090, local=%d\n", locally_generated);
