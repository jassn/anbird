...
wpa_printf(MSG_WARNING, "js3n.wpa ...... %s:%d", __func__, __LINE__);
...
{
	wpa_dbg(wpa_s, MSG_INFO, "Cancelling authentication timeout");
wpa_msg(wpa_s, MSG_INFO, "js3n.cancel ..... line.249");
	eloop_cancel_timeout(wpa_supplicant_timeout, wpa_s, NULL);
...
static void wpas_start_assoc_cb(struct wpa_radio_work *work, int deinit)
{
	...
	if (bss &&
		...
		wpa_printf(MSG_INFO, "js3n Trying to associate with SSID '%s'",
			wpa_ssid_txt(ssid->ssid, ssid->ssid_len));
...
wpa_msg(wpa_s, MSG_INFO, "jssn, wpa.2689");

	if (addr) {
		wpa_msg(wpa_s, MSG_INFO, "jssn, wpa.2692");
...
wpa_msg(wpa_s, MSG_INFO, "js2n, wpa.5565");
	wpas_connect_work_done(wpa_s);
