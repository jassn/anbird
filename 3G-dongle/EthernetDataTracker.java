...
public class EthernetDataTracker implements NetworkStateTracker {
		...
		if (mIface.matches(iface)) {
			...
			if (up) {
 			...
 			if (up) {
                pppoe_ok = true;
                if(mLinkUp) {
                    Log.e(TAG," ppp0 is up! remove eth0 ip and route!");
    				try{
    					mNMService.clearInterfaceAddresses(mIface);
    					NetworkUtils.resetConnections(mIface, NetworkUtils.RESET_ALL_ADDRESSES);
    				} catch (RemoteException e) {
                ...
                if(pppoe_ok) {
                    pppoe_ok = false;
                    Log.e(TAG," ppp0 is down! reconnect eth0! mLinkUp="+mLinkUp);
    				reconnect();                    
                    if(!mLinkUp) {
                        mLinkUp = up;
                        mContext.sendBroadcast(new Intent(EthernetManager.ETHERNET_LINKED_ACTION));
                    }
                    Log.e(TAG," check : mLinkUp="+mLinkUp);
                }
            }
        }
	}
    ...
    private void runDhcp() {
        Thread dhcpThread = new Thread(new Runnable() {
            public void run() {
                synchronized(this) { 
                Log.d(TAG, "runDhcp === begin ===");

                // DhcpResults dhcpResults = new DhcpResults(mLinkProperties);
                DhcpInfoInternal dhcpResults = new DhcpInfoInternal();
                ...
                if (!NetworkUtils.runDhcp(mIface, dhcpResults)) {
                    ...
                    Log.d(TAG, "runDhcp === end ===");
                    return;
                }

                // mLinkProperties = new LinkProperties(dhcpResults.linkProperties);
                mLinkProperties = dhcpResults.makeLinkProperties();
                if(mLinkProperties.getDnses().size() == 0)
                    for (InetAddress i : dhcpResults.makeLinkProperties().getDnses()) mLinkProperties.addDns(i);

                //Log.d(TAG, "get Dns size " + dhcpResults.linkProperties.getDnses().size());
                //Log.d(TAG, "get Dns size " + mLinkProperties.getDnses().size());
	...
	public void ConnectNetwork(boolean up){
		Log.e(TAG, "ConnectNetwork() : Up is " + up + " mLinkUp is " + mLinkUp +
					" On is " + mEthManage.isOn() + " mIface " + mIface);
		if(!mEthManage.isConfigured()){
            Log.d(TAG, "debug : ConnectNetwork return here.. -> up="+up);
			return;
		}
		if(up && mEthManage.isOn()){
			...
			if(mEthManage.isDhcp()){
				try{
					Log.e(TAG, "ConnectNetwork : isDhcp, clear NMService IP here ---------------------------------");
					//NetworkUtils.stopDhcp(mIface);
					mLinkProperties.clear();
					mNetworkInfo.setIsAvailable(false);
					mNMService.clearInterfaceAddresses(mIface);
					...
					Log.e(TAG, "ConnectNetwork: ERROR: " + e);
				}
                ...
                String dlnaFakIpEnable = SystemProperties.get("rtk.dlna.eth0.fakeip.enable");
                Log.d(TAG, "ConnectNetwork : check rtk.dlna.eth0.fakeip.enable="+dlnaFakIpEnable);
				runDhcp();
			}else{
				//NetworkUtils.stopDhcp("eth_" + mIface);
				Log.e(TAG, "ConnectNetwork : if fixed ip--------------------------------");
				NetworkUtils.stopDhcp(mIface);
                ...
                Log.e(TAG, "ConnectNetwork : mLinkProperties="+mLinkProperties);
			...
			//NetworkUtils.stopDhcp("eth_" + mIface);
			Log.d(TAG, "ConnectNetwork : link down... stopDhcp here...");
			NetworkUtils.stopDhcp(mIface);
			mLinkProperties.clear();
			mNetworkInfo.setIsAvailable(false);   
			mNetworkInfo.setDetailedState(DetailedState.DISCONNECTED, null, null);
